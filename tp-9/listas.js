...
function encontrarPalabraMasCorta(listaDePalabras) {
  ...
  return listaDePalabras.reduce((corta, palabra) =>
    palabra.length < corta.length ? palabra : corta
  );
}
...
function enumerarLista(listaDePalabras) {
  ...
  const todasMenosUltima = listaDePalabras.slice(0, -1).join(", ");
  const ultima = listaDePalabras[listaDePalabras.length - 1];
  
  const oracion = `${todasMenosUltima} y ${ultima}.`;
